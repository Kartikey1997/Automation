...
class Test_Enquiry1:
    ...
    def test_001_RED_ButtonInquiry(self,setup):                  ############Always define a testcase function with its initials as "test"


        self.logger.info("Test_001_RED_ButtonInquiry ************")
        self.driver = setup
        ...
        if (revertmsg == "Thank you"):
            ...
            self.logger.info("============ Test_001_RED_ButtonInquiry, PASSED ============")
            assert True
        else:
            self.logger.info("============= Red Button Inquiry not Submitted, screenshot generated ============")
            self.logger.info("============ Test_001_RED_ButtonInquiry, FAILED ============")
            timestr = datetime.now().strftime("%d-%m-%Y--%I:%M:%p")
